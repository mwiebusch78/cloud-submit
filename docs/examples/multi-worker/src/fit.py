...
def fit_step(
    ...
    print(f'Fitting model. Worker index: {worker_index}')

    # Do K-fold cross validation. Each worker does one fold.
    data = pl.read_parquet(train_data_path)
    splits = np.round(np.linspace(0, len(data), num_workers + 1))
    fold_start = int(splits[worker_index])
    fold_end = int(splits[worker_index + 1])
    ...
    path = os.path.join(coefficients_path, f'fold{worker_index}.json')
    ...
    path = os.path.join(predictions_path, f'fold{worker_index}.parquet')
